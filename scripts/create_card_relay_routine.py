...
STEP 3 - Write ONE Google Drive file with the Google Drive connector, using create_file:
  ...
  - AN EMPTY RESULT DOES NOT PROVE THE FILE IS ABSENT. Drive's index is eventually consistent: in that same test a renamed file kept matching its OLD title for roughly two minutes. If your filtered search comes back empty, SEARCH THREE TIMES IN TOTAL, doing something else in between — re-read the card, re-check the title you expect — and only then conclude the card has not been relayed. Do NOT write an instruction to yourself to sleep for a fixed period: the sandbox blocks a standalone `sleep 60`, and a sibling relay that tried it backgrounded the sleep and then ran `wait <task-id>` on something that was not a shell job, so `wait` returned instantly and the "60-second" retry happened 14 seconds later. Repeating the search needs no timing primitive and cannot be defeated that way. Concluding "not yet relayed" from a stale empty result writes a duplicate; concluding it as a reader makes a working pipeline look broken.
