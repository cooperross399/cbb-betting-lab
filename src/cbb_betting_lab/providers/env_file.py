...
This exists so a local operator can keep a credential out of shell history
without weakening the credential rules. It is deliberately narrow:
...
def redact(text: str, environment: dict[str, str] | None = None) -> str:
    ...
    first place; this makes a mistake in that direction non-fatal. Both the
    query-parameter form (The Odds API) and the bearer-header form
    (CollegeBasketballData) are scrubbed, because the two providers leak
    differently and only one of them was in the sibling labs.
    """
    ...
    cleaned = re.sub(r"(apiKey=)[^&\s\"']+", r"\1[redacted]", cleaned)
    return re.sub(
        r"(?i)(authorization:\s*bearer\s+)\S+", r"\1[redacted]", cleaned
    )
