...
place this file writes either. Its composition is — 30 discovery entries, the 32 holdout looks the replication
...
was registered under — and every interval is widened by the factor the block states. The
...
**`docs/project_status.md` and this file are part-generated now.** The per-tier
headline in both is spliced from the record by
`scripts/splice_headline_table.py`, so run that too — it is the one figure a
reader acts on and it no longer depends on anyone retyping it. The prose around
it is still hand-written, and every interval in it either matches a current
reading, carries an explicit `[@N]` saying which correction it is stated at, or
is refused.
...
**`total_points` / mid-major** cell, -6.4% over 8,214 bets, read a demonstrated deficit at -12.5% to -0.2% [@62] at those same 62
hypotheses and reads **no demonstrated edge** today; it crossed at the
**95th**, two registrations ago. Its current interval is in the generated
block, which is the only place this file states one. Five blind null-baseline sides lose a
...
same five.** (At today's count they are 70, 61 and 27. The counts in this
...
hypotheses that record was scored at and none of them demonstrated at today's count:
...
**Nine more went at 101 -> 128, and this registration is what spent them.**
Putting the 27 residual-regression features on the ledger on 2026-09-15 took the
...
clearing the bar inside that one step. All nine are blind null-baseline sides:
`high_major / player_assists / always the underdog` crossed at the 111th
hypothesis, `high_major / player_rebounds / always the favourite` at the 123rd,
`high_major / spread / always away` at the 125th, and
`mid_major / player_rebounds_assists / always the favourite` at **the last of
the 27** -- the final hypothesis registered here is the one that withdrew it. The first three
appear in both the full store and the held-out backtest; the `spread` side
appears in the core-team cut as well, which is why the table holds nine rows for
four readings. `docs/retracted_readings.md` states each crossing point and is
...
- **The model does not beat blind betting on return, and it loses to the vig.**
  ...
  their own tier's model**, and 10 of those carry a demonstrated deficit at today's correction (13 at the x1.6041 the record was FIRST scored at, before its
  ...
  the model's own mid-major return in the table above. (This named `always the underdog`, which carries the same
...
- **The skill measure shows nothing.** The disagreement coefficient over every
  ...
  Every interval spans zero; they are in the generated block. On the **selected** bets only (110,316) high-major reads a demonstrated
  edge — that is the winner's-curse comparison, never the skill measure, and
  the figure is in the generated block with the rest. **Two registrations moved
  ...
  to +0.016 at 62, and the 33 player-prop hypotheses took it lower again at 95; the coefficient's current reading is in
`data/outputs/cbb_forecast_skill.md`, which re-renders. This file previously credited the whole 30-to-95 move to the
