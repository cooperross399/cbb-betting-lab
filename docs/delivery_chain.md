...
## Link 3 is blocked, and exactly what the block is

**Status 2026-09-03: the `CBB CARD RELAY` routine does not exist, because the
cloud environment has no access to this repository.** Creating it returns:

    HTTP 403  "You don't have access to a repository this routine uses."

The routine body is correct and is generated by
`scripts/create_card_relay_routine.py` — same environment, same Google Drive
connector and same shape as the three working sibling relays. Only the grant
is missing.

**This was proven rather than assumed, which the brief asks for explicitly.**
The obvious next thought is that Claude could add the grant itself through the
GitHub App installation API (`PUT /user/installations/{id}/repositories/{id}`).
It cannot:

    gh api /user/installations
    403  "You must authenticate with an access token authorized to a GitHub
          App in order to list installations"

`gh` here holds an OAuth token with `repo`, `workflow`, `gist` and `read:org`.
Modifying a GitHub **App** installation needs a user-to-server token issued by
that App, which no tool in this environment can mint. So the grant is Cooper's
to make and there is no route around it.

**What Cooper does — once, and it is not the chat-task paste.** Add
`cooperross399/cbb-betting-lab` to the Claude GitHub App's repository access,
then re-run:

    PYTHONPATH=src .venv/bin/python scripts/create_card_relay_routine.py

and POST that body with `RemoteTrigger create`. Nothing else about the chain
changes; links 1 and 2 are already verified against real commits on
`card-feed`.

**Until then the lab is not silent — it is one link short.** The workflow
publishes, `card-feed` carries a real card, and the card is readable from the
run page and from the branch. What does not happen is the copy into Drive, so
the chat-side task has nothing to read. That is a delivery gap, not a pipeline
gap, and the two must not be reported as the same thing.
